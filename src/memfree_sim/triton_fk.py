...
try:
    ...
    HAS_TRITON = True
except ImportError:  # pragma: no cover - exercised in non-Linux development environments.
    triton = None
    tl = None
    HAS_TRITON = False
...
if HAS_TRITON:
    ...
    def _fk_forward_kernel(
        ...
        for joint_idx in range(7):
            theta = tl.load(q_ptr + env_offsets * q_stride0 + joint_idx, mask=mask, other=0.0)
        ...
        tl.store(out_ptr + env_offsets * out_stride0 + 0 * out_stride1 + 0 * out_stride2, r00, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 0 * out_stride1 + 1 * out_stride2, r01, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 0 * out_stride1 + 2 * out_stride2, r02, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 0 * out_stride1 + 3 * out_stride2, tx, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 1 * out_stride1 + 0 * out_stride2, r10, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 1 * out_stride1 + 1 * out_stride2, r11, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 1 * out_stride1 + 2 * out_stride2, r12, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 1 * out_stride1 + 3 * out_stride2, ty, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 2 * out_stride1 + 0 * out_stride2, r20, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 2 * out_stride1 + 1 * out_stride2, r21, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 2 * out_stride1 + 2 * out_stride2, r22, mask=mask)
        tl.store(out_ptr + env_offsets * out_stride0 + 2 * out_stride1 + 3 * out_stride2, tz, mask=mask)
        tl.store(
...
def _prepare_inputs(q: torch.Tensor, arm_spec: ArmSpec) -> tuple[torch.Tensor, ArmSpec]:
    if not HAS_TRITON:
        raise RuntimeError("Triton is not available. Install this project on Linux/WSL with the triton package.")
    if not q.is_cuda:
